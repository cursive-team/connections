...
export function round1_js(gen_keys_output, other_message_round1, bit_vector) {
  ...
  const ret = wasm.round1_js(
    addHeapObject(gen_keys_output),
    addHeapObject(other_message_round1),
    ptr0,
    len0
  );
  return takeObject(ret);
 ...
 * @returns {any}
 */
export function round2_js(
  gen_keys_output,
  round1_output,
  other_message_round2,
  is_a
) {
  const ret = wasm.round2_js(
    addHeapObject(gen_keys_output),
    addHeapObject(round1_output),
    addHeapObject(other_message_round2),
    is_a
  );
  return takeObject(ret);
}
...
export function round3_js(round2_output, other_message) {
  try {
    ...
    wasm.round3_js(
      retptr,
      addHeapObject(round2_output),
      addHeapObject(other_message)
    );
