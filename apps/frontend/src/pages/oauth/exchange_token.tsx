    ...
    const fetchAccessToken = async () => {
      ...
      if (code && state) {
        ...
        if (accessToken && details.can_import) {
          ...
          const leaderboardEntry = await importOAuthData(user.userData.username, ChipIssuer.EDGE_CITY_LANNA, accessToken, importOption);

          try {
            if (!leaderboardEntry) {
              throw new Error("Imported leaderboard entry is null");
            }
            await updateLeaderboardEntry(leaderboardEntry);
